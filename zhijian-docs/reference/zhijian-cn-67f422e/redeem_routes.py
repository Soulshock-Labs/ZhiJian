...
def register_redeem_routes(
    app: FastAPI,
    *,
    load_redeem_codes: Callable[[], dict[str, dict]],
    redeem_code_core: Callable[..., dict[str, Any]],
    utc_iso: Callable[[], str],
    dispatch_webhook: Callable[..., Any],
    partner_redeem_tokens: set[str],
    partner_redeem_source: str,
    partner_webhook_urls: dict[str, str],
    generate_unique_code: Callable[..., str],
    save_redeem_codes: Callable[[dict[str, dict]], None],
    logger: Any,
) -> None:
    router = APIRouter(tags=["兑换"])
        ...
        if result.get("ok"):
            ...
            if callback_url:
                webhook_body = {
                    ...
                    "redeemed_at_utc": utc_iso(),
                    "status": "redeemed",
                    "channel": "user_self",
                }
                asyncio.create_task(dispatch_webhook(callback_url, webhook_body, cb_order_id, code))
        else:
            result.pop("_callback_url", None)
            result.pop("_order_id", None)
        return result

    @router.post("/partner/redeem")
    ...
    @router.post("/partner/create-code")
    async def partner_create_code(
        ...
        codes = load_redeem_codes()
        new_code = generate_unique_code(codes, service_type)
        codes[new_code] = {
            "code": new_code,
            "status": "unused",
            "token_type": "auto",
