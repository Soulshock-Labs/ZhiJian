...
def extract_doc_context(file_bytes: bytes, filename: str) -> DocContext:
    """
    从上传文件提取文档上下文。
    支持 .docx 和 .pdf，失败时返回 ok=False 的空 DocContext。
    """
    ctx = DocContext(filename=filename)
    ext = (filename or "").lower().rsplit(".", 1)[-1]
    ctx.file_type = ext if ext in {"docx", "pdf"} else "unknown"
    ...
    try:
        if ctx.file_type == "docx":
            ...
            ctx.ok = False
            ctx.error = f"不支持的文件格式：{ext}，仅支持 .docx 和 .pdf"
            return ctx
